...
import AddUpiNetBanking from "../snippets/addUpiNetBanking";
import { useState } from "react";

function UpiSec() {
  let savedUpi = [1, 1, 2, 3, 4];
  const [open, setOpen] = useState(false);

  return (
    <>
      <h3 className="product_title">Save Cards</h3>
      <div className="grid upi_sec_options_box">
        {savedUpi.map((e, index) => {
          return (
            <div key={index} className="upi_ids flex">
              <img
                className=""
                src={require(`../../assets/icons/phonepe.svg`).default}
                alt=""
              />
              <span className="upi_id">...xxxx2542</span>
      ...
      {open && <AddUpiNetBanking formHeading="add Upi" label={'save Upi'} />}
    </>
  );
}

export default UpiSec;
